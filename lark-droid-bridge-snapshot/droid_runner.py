"""
通过 subprocess 调用 Factory Droid CLI，解析 stream-json 输出。
"""
...
from bot_config import PERMISSION_MODE, DROID_CLI
...
async def run_droid(
    ...
    async def _run_once(active_session_id: Optional[str]) -> tuple[str, Optional[str], int, str]:
        ...
        if os.environ.get("DROID_SKIP_PERMISSIONS_UNSAFE", "1") not in {"1", "true", "yes"}:
            if mode == "plan":
                cmd += ["--auto", "low"]
            else:
                cmd += ["--auto", os.environ.get("DROID_AUTO", "medium")]
        if active_session_id:
            cmd += ["--session-id", active_session_id]
        if model:
            cmd += ["-m", model]
    ...
    if session_id and returncode != 0 and not stderr_text and not final_text:
        print("[run_droid] resume failed without stderr, retrying with fresh session", flush=True)
    ...
    if returncode != 0:
        ...
        raise RuntimeError(f"droid exited with code {returncode}: {detail}")
